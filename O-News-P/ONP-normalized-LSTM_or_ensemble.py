...
plt.plot(aa, prediction[-200:,], 'g', label="prediction")
...
plt.plot(indexes,[prediction[-200:,][i] for i in indexes], ls="", marker="o", label="actual anomalies",alpha=0.6)
p_indexes = [index for index in range(len(outcome200)) if outcome200[index] == 0]
plt.plot(p_indexes,[prediction[-200:,][i] for i in p_indexes], ls="", marker="*", label="predicted anomalies")
